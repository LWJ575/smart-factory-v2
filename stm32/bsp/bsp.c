...
/* ============================================================
 * BSP 实现 — STM32F103ZET6 (SPL, FreeRTOS)
 * ============================================================ */

/* ---- pvPortRealloc: FreeRTOS 堆上的 realloc (供 LVGL 使用) ---- */
void *pvPortRealloc(void *ptr, size_t new_size)
{
    ...
    /* 不知旧块大小, 拷贝 new_size 字节.
     * 缩小: 安全; 放大: 会读到下一块元数据, Cortex-M3 无 MMU 不会 fault,
     * 调用方拿到新块后会先写入再读, 尾部垃圾字节不影响逻辑 */
...
/* FreeRTOS 接管 SysTick, 所以 tick 计数用 xTaskGetTickCount()
 * 但在中断里不方便调 API, 这里用一个全局变量做备份 */
static volatile uint32_t s_tick_ms = 0;

/* 在 vApplicationTickHook 中调用 */
...
{
    /* BKP_DR1 ~ BKP_DR10, SPL 通过 BKP_WriteBackupRegister 写 */
    /* 简单映射: BKP_DR1=0, DR2=1, ... */
    switch (dr) {
