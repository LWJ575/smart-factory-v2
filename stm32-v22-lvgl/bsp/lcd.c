 ...
 * 正点原子精英版: PB0 背光, FSMC NE4(PG12) 片选 + A10(PG0) 做 RS
 * ============================================================ */
...
static void fsmc_init(void)
{
    ...
    /* GPIOG: NE4=PG12 (LCD 片选), A10=PG0 (LCD_RS 信号线!) */
...
/* 读取 LCD 控制器 ID (ILI9341 应返回 0x9341)
 * 用于验证 FSMC 总线通信是否正常 */
uint32_t lcd_read_id(void)
{
    uint16_t id_hi, id_lo;

    LCD_REG = 0xD3;             /* Read ID4 命令, 返回 4 个参数 */
    (void)LCD_RAM;              /* P1 = 0x00 (dummy) */
    (void)LCD_RAM;              /* P2 = 0x00 (dummy) */
    id_hi = LCD_RAM;            /* P3 = 0x93 */
    id_lo = LCD_RAM;            /* P4 = 0x41 */
    /* 16 位总线上参数出现在低 8 位, 高 8 位为 0 */
    return ((uint32_t)(id_hi & 0xFF) << 8) | (id_lo & 0xFF);
}
