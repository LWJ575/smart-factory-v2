                 ...
                 const char *will_msg)
{
    ...
    while (retries-- > 0) {
        ...
        /* 等待 CONNACK (最多 5 秒, 累积接收防分片)
         * 4 字节的 CONNACK 可能分 2 次串口中断到达,
         * 必须攒齐再解码, 否则每次都解失败 */
        uint32_t start = sys_tick();
        int acc_len = 0;
        while (sys_tick() - start < 5000) {
            int n = esp8266_recv_tcp(s_rx_buf + acc_len,
                                     sizeof(s_rx_buf) - acc_len);
            if (n > 0) {
                acc_len += n;
            ...
            if (acc_len >= (int)sizeof(s_rx_buf)) break;  /* 缓冲区满 */
        }
...
int mqtt_subscribe(const char *topic)
{
    ...
    while (sys_tick() - start < 3000) {
        ...
        if (n > 0) {
            ...
            /* 如果收到 PUBLISH 也处理( SUBACK 和 PUBLISH 可能交叉到达) */
        }
        if (acc_len >= (int)sizeof(s_rx_buf)) break;  /* 缓冲区满 */
    }
