...
/* ============================================================
 * LVGL 显示驱动移植实现 (v2.2, 无 FreeRTOS)
 * - 双 partial buffer (每次 LVGL_DISP_BUF_LINES 行, 见 config.h)
 * - flush 回调直接写 LCD GRAM
 * - tick: lv_conf.h 中 LV_TICK_CUSTOM=1 直读 sys_tick(), 无需 hook
 * ============================================================ */

/* 显示缓冲区 (RGB565, 每行 320 像素 = 640 字节)
